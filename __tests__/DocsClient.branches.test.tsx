...
describe('DocsClient targeted branches', () => {
  ...
  it('renders feedback controls for agent messages and marks submitted feedback', async () => {
    ...
    global.fetch = jest.fn((input: RequestInfo, init?: RequestInit) => {
      ...
      if (url.includes('/sessions/') && (!init || init.method === 'GET')) {
        ...
        return Promise.resolve({ ok: true, json: async () => ({ status: 'success', data: { messages: [ { id: 'u1', sender: 'user', content: 'user prompt' }, { id: 'm1', sender: 'assistant', content: 'agent reply' } ] } }) }) as any
  ...
  it('falls back to default title when widgetConfig title is missing', async () => {
    ...
    const { getByText } = render(<DocsClient clientId="c" agentId="a" configId="cfg" locale="en" startOpen={true} />)
  ...
  it('does not post hide/show when running in the top window', async () => {
    ...
    render(<DocsClient clientId="c" agentId="a" configId="cfg" locale="en" startOpen={true} />)
